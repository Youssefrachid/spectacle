...
if ($_SERVER['HTTP_REFERER'] == 'http://localhost/spectacle/admin/form.php') { // vérifie qu'on vient bien du formulaire
    ...
    if (empty($price) || strlen($price) > 999.99) {
    ...
    if (empty($_FILES['img']['name']) || $_FILES['img']['size'] > 2000000 || !in_array($_FILES['img']['type'], $authorizedFormats)) {
        $errorMessage .= '<p>- l\'image est obligatoire, ne doit pas dépasser 3 Mo et doit être au format PNG, JPG, JPEG, JP2 ou WEBP.</p>';
        $validation = false;
    }
